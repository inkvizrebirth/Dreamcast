...
public final class ClickGuiScreen extends Screen {
	...
	@Override public void extractRenderState(GuiGraphicsExtractor g,int mx,int my,float delta){
		super.extractRenderState(g,mx,my,delta);paletteScroll+=(paletteTarget-paletteScroll)*.22F;inspectorScroll+=(inspectorTarget-inspectorScroll)*.22F;float appear=Math.min(1F,(Util.getMillis()-openedAt)/260F);g.pose().pushMatrix();g.pose().translate(0,(1-smooth(appear))*7);
	...
	private void drawLibrary(GuiGraphicsExtractor g,int mx,int my){
		...
		for(AutomationConfig c:AutomationManager.all()){
			...
			button(g,run,AutomationRunner.isRunning(c)?"Стоп":"Запуск",run.has(mx,my),AutomationRunner.isRunning(c)?0xFFFF6B78:0xFF66D9A3,true);
	...
	private void drawEditor(GuiGraphicsExtractor g,int mx,int my){
		top(g,"Конструктор автоматизации");Box back=new Box(14,12,58,24),save=new Box(width-214,12,76,24),run=new Box(width-130,12,72,24),stop=new Box(width-52,12,38,24);
		button(g,back,"Назад",back.has(mx,my),MUTED,false);button(g,save,"Сохранить",save.has(mx,my),ACCENT,false);
		button(g,run,"Запуск",run.has(mx,my),0xFF66D9A3,true);button(g,stop,"■",stop.has(mx,my),0xFFFF6B78,false);
		Box name=nameBox();field(g,name,editing.name,"name".equals(focusedField),mx,my);
		Box legit=legitBox();button(g,legit,editing.legit?"✓  Легит":"Легит",legit.has(mx,my),0xFF66D9A3,editing.legit);
		drawCanvas(g,mx,my);drawPalette(g,mx,my);drawInspector(g,mx,my);
	}
	...
	private void drawInspector(GuiGraphicsExtractor g,int mx,int my){
		...
		g.enableScissor(x+1,TOP+68,width,height-50);int y=TOP+76-Math.round(inspectorScroll);for(Map.Entry<String,String> e:selected.values.entrySet()){String id="value:"+e.getKey();RenderUtils.textFlat(g,font,label(e.getKey()),x+16,y,MUTED);
			Box f=new Box(x+16,y+13,RIGHT-32,24);field(g,f,e.getValue(),id.equals(focusedField),mx,my);y+=49;}
		g.disableScissor();
	...
	private boolean clickLibrary(double mx,double my,int button){
		if(button!=GLFW.GLFW_MOUSE_BUTTON_LEFT)return true;if(new Box(20,12,142,25).has(mx,my)){editing=AutomationManager.create();return true;}
	...
	private String summary(AutomationNode n){return switch(n.type){case START->"Начало потока";case STOP->"Конец потока";case GOTO,OPEN->trim(n.value("x")+", "+n.value("y")+", "+n.value("z"),128);case MINE->trim(n.value("block")+" × "+n.value("count"),128);case SEARCH->trim(n.value("block"),128);case USE,INTERACT->"Рука: "+n.value("hand");case SELECT_SLOT->"Слот "+n.value("slot");case MOVE_ITEM->n.value("from")+" → "+n.value("to");case QUICK_MOVE->"Слот "+n.value("slot");case DROP_ITEM->"Слот "+n.value("slot")+" • "+n.value("amount");case TAKE_CONTAINER->"Пауза "+n.value("delay_ticks")+" тик.";case EAT->trim(n.value("food"),128);case FOOD_CHECK,HEALTH_CHECK->n.value("operator")+" "+n.value("value");case ITEM_CHECK->trim(n.value("item")+" × "+n.value("count"),128);case LOOK->"point".equals(n.value("mode"))?"К точке":"Yaw "+n.value("yaw")+" • Pitch "+n.value("pitch");case MOVE->n.value("direction")+" • "+n.value("seconds")+" сек.";case JUMP->"Обычный прыжок";case SNEAK->n.value("seconds")+" сек.";case ATTACK->n.value("swings")+" удар.";case FOLLOW->trim(n.value("entity")+" "+n.value("name"),128);case EXPLORE,FARM->"Радиус "+n.value("radius");case COORDINATE_CHECK->n.value("axis")+" "+n.value("operator")+" "+n.value("value");case CHAT->trim(n.value("message"),128);case WAIT->n.value("seconds")+" сек.";case COMMAND->trim(n.value("command"),128);case SET_VARIABLE->trim(n.value("name")+" = "+n.value("value"),128);case CONDITION->trim(n.value("left")+" "+n.value("operator")+" "+n.value("right"),128);};}
	private String label(String k){return switch(k){case"x","y","z"->"Координата "+k.toUpperCase();case"axis"->"Ось: x / y / z";case"block"->"ID блока";case"item"->"ID предмета";case"count"->"Количество";case"slot"->"Слот / слот хотбара";case"from"->"Из слота меню";case"to"->"В слот меню";case"amount"->"one / stack";case"delay_ticks"->"Пауза между кликами";case"food"->"ID еды или any";case"restore_slot"->"Вернуть слот: true / false";case"mode"->"angles / point";case"yaw"->"Yaw";case"pitch"->"Pitch";case"direction"->"forward / back / left / right";case"swings"->"Количество ударов";case"hand"->"Рука: main / off";case"entity"->"Тип сущности";case"radius"->"Радиус";case"message"->"Сообщение";case"seconds"->"Секунды";case"command"->"Команда Baritone";case"name"->"Имя / переменная";case"value"->"Значение";case"left"->"Левая часть";case"operator"->"Оператор: == != > >= < <= contains";case"right"->"Правая часть";default->k;};}

	private void field(GuiGraphicsExtractor g,Box b,String value,boolean focus,int mx,int my){float h=anim("field:"+b.x+":"+b.y,b.has(mx,my)||focus);RenderUtils.fillRoundedBorder(g,b.x,b.y,b.w,b.h,7,RenderUtils.mix(BORDER,ACCENT,h),RenderUtils.mix(0xB30A0C12,0xD31B1D28,h*.45F));String shown=trim(value,b.w-18);if(focus&&(Util.getMillis()/500L)%2==0)shown+="|";RenderUtils.textFlat(g,font,shown,b.x+9,b.y+7,RenderUtils.mix(MUTED,TEXT,.45F+h*.55F));}
